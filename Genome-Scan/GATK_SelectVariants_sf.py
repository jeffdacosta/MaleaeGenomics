...
parser = argparse.ArgumentParser(description='This script uses the GATK SelectVariants function to extract data for a set of samples '+
'(listed in the sf file) from vcf files. A separate slurm shell script is created and sent to Odyssey for each input vcf file. Output vcf files '+
...
parser.add_argument('-sf', type=str, metavar='sf_file', required=True, help='REQUIRED: Full path to file with list of samples')
parser.add_argument('-suffix', type=str, metavar='suffix', default='sf', help='Suffix to append to output filename [sf]')
...
for in_vcf in in_vcf_list:
    ...
    sh_file.write('#!/bin/bash\n'+
                  ...
                  ' -sf '+args.sf+' -o '+args.o+basename+'_'+args.suffix+'.vcf\n'+
                  'printf "\\nFinished\\n\\n"\n')
    sh_file.close()
