...
for file in os.listdir(args.i):
    if file.endswith('dedupNamed.bam'):
        bam_list.append(file)
bam_list.sort()

print('\nFound '+str(len(bam_list))+' deduplicated and name-fixed bam files:')
...
count = 0
for i in range(len(bam_list)):
    basename = bam_list[i].replace('_dedupNamed.bam','')

    sh_file = open(args.o+basename+'.sh','w')

    #write slurm shell file
    sh_file.write('#!/bin/bash\n'+
                  '#SBATCH -J IR.'+basename+'\n'+
                  ...
                  'java -Xmx8g -jar /n/sw/GenomeAnalysisTK-2.7-2-g6bda569/GenomeAnalysisTK.jar -nt '+args.nt+' -T RealignerTargetCreator -R '+args.ref+
                  ' -I '+args.i+bam_list[i]+' -o '+args.o+basename+'_IndelRealigner.intervals\n'+
                  'java -Xmx8g -jar /n/sw/GenomeAnalysisTK-2.7-2-g6bda569/GenomeAnalysisTK.jar -nt 1 -T IndelRealigner -R '+args.ref+
                  ' -targetIntervals '+args.o+basename+'_IndelRealigner.intervals -I '+args.i+bam_list[i]+' -o '+args.o+basename+'_realigned.bam\n'+
                  'printf "\\nFinished\\n\\n"\n')
    sh_file.close()
