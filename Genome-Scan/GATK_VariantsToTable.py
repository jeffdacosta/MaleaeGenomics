...
parser = argparse.ArgumentParser(description='This script uses the GATK VariantsToTable function to generate allele frequency tables from '+
'vcf files. Four columns are output for each variant: chromosome (CHROM), position (POS), allele count (AC), and depth. '+
...
parser.add_argument('-suffix', type=str, metavar='suffix', default='raw', help='Suffix to append to output filename [raw]')
...
for in_vcf in in_vcf_list:
    ...
    sh_file.write('#!/bin/bash\n'+
                  ...
                  'java -Xmx12g -jar /n/sw/GenomeAnalysisTK-2.7-2-g6bda569/GenomeAnalysisTK.jar -T VariantsToTable -V '+args.i+in_vcf+' -R '+args.R+
                  ' -F CHROM -F POS -F AC -F DP -raw --allowMissingData -o '+args.o+basename+'_'+args.suffix+'.table\n'+
                  'printf "\\nFinished\\n\\n"\n')
    sh_file.close()
