...
parser = argparse.ArgumentParser(description='This script uses picard-tools to remove duplicate sequences from sorted bam files. '+
'It searches for files ending in sorted.bam in the input directory, and a separate slurm shell script is created and sent to Odyssey for each input file. '+
' One dedup.bam and duplicateMetrics.txt file is generated in the output direcotry for each input file. ')
...
print('\nFound '+str(len(bam_list))+' sorted bam files:')
...
count = 0
for i in range(len(bam_list)):
    basename = bam_list[i].replace('.sort.bam','')
    ...
    sh_file.write('#!/bin/bash\n'+
                  ...
                  'java -Xmx4g -jar /n/sw/picard-tools-1.98/MarkDuplicates.jar I='+args.i+bam_list[i]+' O='+args.o+basename+'_dedup.bam M='+args.o+basename+'_duplicateMetrics.txt '+ 
